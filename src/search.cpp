         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta = Value(16) + int(prev) * prev / 19837;
              alpha = std::max(prev - delta,-VALUE_INFINITE);
              beta  = std::min(prev + delta, VALUE_INFINITE);

              // Adjust optimism based on root move's previousScore
              int opt = 113 * prev / (std::abs(prev) + 160);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // return a fail low.
    if (eval < alpha - 394 - 237 * depth * depth)
    {
    ...
    if (   !ss->ttPv
        &&  depth < 8
        &&  eval - futility_margin(depth, improving) - (ss-1)->statScore / 263 >= beta
        &&  eval >= beta
        &&  eval < 24519) // larger than VALUE_KNOWN_WIN, but smaller than TB wins.
        return eval;

    // Step 8. Null move search with verification search (~22 Elo)
    if (   !PvNode
        && (ss-1)->currentMove != MOVE_NULL
        && (ss-1)->statScore < 14294
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 16 * depth - improvement / 15 + 100 + complexity / 24
        && !excludedMove
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = std::min(int(eval - beta) / 151, 5) + depth / 3 + 4 - (complexity > 667);
    ...
    probCutBeta = beta + 210 - 47 * improving;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_MATED_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              if (   !givesCheck
                  ...
                  && lmrDepth < 6
                  && !ss->inCheck
                  && ss->staticEval + 294 + 192 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))]
                   + captureHistory[movedPiece][to_sq(move)][type_of(pos.piece_on(to_sq(move)))] / 6 < alpha)
                  continue;

              // SEE based pruning (~9 Elo)
              if (!pos.see_ge(move, Value(-235) * depth + Value(43)))
                  continue;
          }
          else
          {
              ...
              if (   !ss->inCheck
                  && lmrDepth < 11
                  && ss->staticEval + 107 + 107 * lmrDepth + history / 80 <= alpha)
                  continue;

              // Prune moves with negative SEE (~3 Elo)
              if (!pos.see_ge(move, Value(-27 * lmrDepth * lmrDepth - 21 * lmrDepth)))
                  continue;
          }
      }
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          // Quiet ttMove extensions (~0 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)] >= 5258)
              extension = 1;
      }
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          if (value > alpha && d < newDepth)
          {
              ...
              const bool doDeeperSearch = value > (alpha + 46 + 11 * (newDepth - d));
              const bool doEvenDeeperSearch = value > alpha + 603 && ss->doubleExtensions <= 5;
